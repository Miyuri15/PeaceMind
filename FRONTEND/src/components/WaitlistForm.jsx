...
const WaitlistForm = () => {
  ...
  const sendData = (e) => {
    ...
    axios.post("http://localhost:8070/waitlist/Waitlistadd", newWaitlist)
      .then(() => {
  ...
  const sendEmailConfirmationEmail = (email) => {
    axios.post("http://localhost:8070/waitlist/Waitlistsend-email", { email })
      .then((response) => {
  ...
  return (
    <div className="content">
      ...
      <div className="waitlist-form-container" style={formContainerStyle}>
        <h2 style={{ fontSize: '1.4em', fontWeight: 'bold' }}>Waitlist Details</h2>
