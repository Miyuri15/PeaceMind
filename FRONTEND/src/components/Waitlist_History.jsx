...
export default function WaitlistHistory() {
  ...
  useEffect(() => {
    function getWaitlists() {
      axios.get('http://localhost:8070/waitlist/Waitlist')
        .then((res) => {
  ...
  const deleteWaitlist = (id) => {
    ...
    axios.delete(`http://localhost:8070/waitlist/Waitlistdelete/${id}`)
      .then(Response => {
  ...
  return (
    <div className="content">
      <div className="table-content">
        <h2 style={{ fontSize: '1.4em', fontWeight: 'bold' }}>Waitlist Patients History</h2>
        {/* Search input */}
          ...
          <tbody>
            {Array.isArray(filteredWaitlistHistory) && filteredWaitlistHistory.map((waitlist, index) => (
              <tr key={waitlist._id}>
                ...
                <td>
  <button onClick={() => window.location.href=`/Waitlistupdate/${waitlist._id}`} className='report-button'>
    Edit
  </button>
</td>
                <td><button type="button" onClick={() => deleteWaitlist(waitlist._id)} className='report-button'>Delete</button></td>
              </tr>
            ))}
          </tbody>
        </table>
