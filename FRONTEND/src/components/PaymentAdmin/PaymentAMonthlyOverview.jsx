...
const AMonthlyOverview = () => {
  ...
  const fetchMonthlyReport = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://localhost:8070/payment/Paymentmonthly', {
        params: {
  ...
  const fetchWeeklyReport = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('http://localhost:8070/payment/Paymentweekly');
      setMonthlyReport(response.data);
    } catch (error) {
...
export default AMonthlyOverview;
