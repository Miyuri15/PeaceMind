...
;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8070/waitlist/Waitlistget/${id}`);
  ...
  const handleSubmit = (e) => {
    ...
    const endpoint = `http://localhost:8070/waitlist/Waitlistupdate/${id}`;
    ...
    axios.put(endpoint, updatedData)
      .then((response) => {
        if (response.status === 200) {
          ...
          navigate('/Waitlist_History'); // Update the formData state with the updated data
        } else {
      ...
      <div style={formContainerStyle}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input type="date" id="waitlistDate" value={formData.waitlistDate} onChange={handleInputChange} style={inputStyle} min={currentDate}disabled />
          </div>
